...
int main(void)
{
	uint32_t * pC1kcrtlreg = (uint32_t*)0x40021018; // Register of the RCC for APB2STR related to PORTC
	uint32_t * pPortDModeReg = (uint32_t*)0x40011004; // Enable the port Register - high register because PIN13
	uint32_t * pPortDOutReg = (uint32_t*)0x4001100C; // Register for the OUTPUT Pins
	...
	*pPortDModeReg &= ~(1<<22); // Quit the reset state of the PC13 and put it in push pull.
	...
	while(1){
		*pPortDOutReg |= (1<<13);
		//*pPortDOutReg |= 0x00002000; Equivalent
		delay(100000);
		*pPortDOutReg &= ~(1<<13);
		//*pPortDOutReg &= ~0x00002000; Equivalent
		delay(100000);
